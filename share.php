...
if ( ! defined( 'SHARE_PLUGIN_URL' ) ) {
	define( 'SHARE_PLUGIN_URL', WP_PLUGIN_URL . '/share' );
}

if ( ! defined( 'SHARE_PLUGIN_DIR' ) ) {
	define( 'SHARE_PLUGIN_DIR', WP_PLUGIN_DIR . '/share' );
}
...
function share_settings_page() {
	wp_enqueue_script( 'share-admin', SHARE_PLUGIN_URL . '/js/share.admin.js', array( 'jquery-ui-sortable' ) );
	wp_enqueue_style( 'share-admin', SHARE_PLUGIN_URL . '/css/share.admin.css' );
...
function share_enqueue_scripts() {
	...
	wp_enqueue_script( 'share', SHARE_PLUGIN_URL . '/js/share.min.js' );
}
...
function share_counts( $url = '', $cache = TRUE ) {
	...
	if ( $cache
	     ...
	     // one hour cache
	     && time() < $shares['_updated'] + 60 * 60
	     // $shares = $networks + 2 (_overall and _updated)
